...
namespace YourTale.Tiles.Plants
{
	public class Daffodil : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			AddMapEntry(new Color(249, 163, 251), name);

			TileObjectData.newTile.CopyFrom(TileObjectData.StyleAlch);
			TileObjectData.newTile.AnchorValidTiles = new int[] {
				TileID.Grass //TODO: Add more tiles soon.
			};
		...
		public override bool CanDrop(int i, int j)/* tModPorter Note: Removed. Use CanDrop to decide if an item should drop. Use GetItemDrops to decide which item drops. Item drops based on placeStyle are handled automatically now, so this method might be able to be removed altogether. */
		{
			...
			int herbItemType = ModContent.ItemType<DaffodilItem>();
			int herbItemStack = 1;

			int seedItemType = ModContent.ItemType<DaffodilSeeds>();
			int seedItemStack = 1;
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			dust = Main.dust[Dust.NewDust(position, 30, 30, DustID.PinkSlime, 0f, 0f, 0, new Color(255, 0, 0), 1f)];
