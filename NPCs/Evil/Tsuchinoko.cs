	...
	internal class TsuchinokoHead : WormHead
	{
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("A mythical creature often sought after for its extremely venomous fangs."),
			});
		}
		...
		{
			float chance = 0;
				if (spawnInfo.SpawnTileY <= Main.maxTilesY && spawnInfo.SpawnTileY >= Main.rockLayer)
				{
					chance += 0.02f;
				}
		...
		public override void AI()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (attackCounter <= 0 && Vector2.Distance(NPC.Center, target.Center) < 200 && Collision.CanHit(NPC.Center, 1, 1, target.Center, 1, 1))
				{
					...
					int projectile = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, direction * 1, Mod.Find<ModProjectile>("PoisonSpit").Type, 5, 0, Main.myPlayer);
